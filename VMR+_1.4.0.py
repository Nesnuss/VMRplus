...
def refdb(name_protein, taxid, parent_class, path, protein_name, min_len, max_len):
    ...
    handle = Entrez.esearch(db="ipg", term=search, retmax=10, usehistory='y', timeout=30)
    record = Entrez.read(handle)
    handle.close()
...
""")
        ...
        #print(new_file)
        term_csv_file = os.path.join(final_dir, term_new_file)
        #print(csv_file)
        term_df_xl.to_csv(term_csv_file, sep=";", index=False)
        logging.info(f"{term_file} to {term_new_file} conversion")
        ...
        final_table_csv = f'VMR+_{new_file}'
        ...
        # Lê o CSV usando o separador ";"
        df_xl_final = pd.read_csv(csv_file, delimiter=';')

        # Define o caminho de saída com extensão .xlsx
